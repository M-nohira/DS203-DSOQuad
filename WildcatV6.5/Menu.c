  ...
  if((Current==TRACK4)&&(_Det==2)){
      if(_4_source>10){
        ...
        if(service)UpdateBackground=3;
          ...
          Print_Str(216,0,0x0405,PRN,(char*)FFT_GAIN_STR[AutoFFT]);
        }
      }else _Det=0;
  }   
}
...
char PPM_Str[5];
  ...
  if((OS_RateSelect)&&(Current==T_BASE)&&(_Det==1)&&(OSBufferLogic())&&(FPGAsubVer)&&(FPGAosFlag)){          //Tbase RANGE sub menu
    Print_Str(99+(3*8),0,0x0405,PRN,(char*)OSrate[OS_Range]);
    Print_Str(98+(12*8),0,0x0405,Twink,"");
  }
  ...
  if((Current==T_BASE)&&(_Det==MODE)&&(_Mode==AUTO)){
    if((AutoTrigSelect)&&(FPGAver>0)){
      if(FreeRunEnable) Print_Str(95+(2*8),0,0x0405,PRN, " AUTO trig CONT WAVES ");
          else Print_Str(95+(2*8),0,0x0405,PRN, "AUTO trig FAST RELEASE");
    } 
  }
...
/*
  }else if((OSBufferLogic())&&(FPGAsubVer)&&(FPGAosFlag)&&(_Tr_kind<8)&&(_Tr_kind>3)){  //also use to grey out non valid OS time trig modes
     if(_T1>_T2){
       if(((_T1-_T2)*FPGA_TbaseOS[Title[T_BASE][BASE].Value].OS_Size)>0xFFFF)GenTrigColorFlag=1;else GenTrigColorFlag=0;
     }else{
       if(((_T2-_T1)*FPGA_TbaseOS[Title[T_BASE][BASE].Value].OS_Size)>0xFFFF)GenTrigColorFlag=1;else GenTrigColorFlag=0;
     }
*/
  }else GenTrigColorFlag=0;
...
void CalculateTvernier(u8 mode){
  ...
  u8 DminBypass=0;
  u16 End=0,IpComp=0,Xposition=_X_posi.Value;
              ...
              if(mode==0){
                ...
                if(mode==2){
                  if((OSBufferLogic())&&(FPGAosFlag==0))End=(_X_posi.Value-StartOffset);else End=0;   
...
s32 k=0;
            ...
            if(mode==0){                                                         //delta V calculations
              ...
              if(_Tr_source==0)k=(_Vt1-_1_posi)*Y_Attr[Title[0][2].Value].SCALE;
                else if((_Tr_source==2)||(_Tr_source==3))k=1000*Y_Attr[0].SCALE; //default approx of 2.0 V trig for V2.81 FPGA digital inputs
...
u8 i;
   ...
   if(AutoSetFlag==1){                                              //initiate
     ...
     if(Title[0][1].Value > 1)Title[0][1].Value = 0;              
   ...
   &&(FrameEndFlag==0)){
     ...
     if(AutoSequence==0){                                       
       ...
       //if(((TaN<3)||(Asignal==0))&&((TbN<3)||(Bsignal==0))&&(_T_base>4)){           
       if(((TaN<2)||(Asignal==0))&&((TbN<2)||(Bsignal==0))&&(_T_base>4)){           
