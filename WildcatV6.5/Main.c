...
#define APP_VERSION "GCCv1.7W6.5 APP(2.51+SmTech1.8+PMOS69 fixes) Custom"
...
DisplayLine DisplayChart[15] = {{(char *)LINE2}, {(char *)LINE3}, {(char *)LINE5}, {(char *)LINE6}, {(char *)LINE7}, {(char *)LINE4}, {(char *)LINE8}, {(char *)LINE9}, {(char *)LINE10}, {(char *)LINE11}, {(char *)LINE12}, {(char *)LINE13}, {(char *)LINE14}, {(char *)LINE15}, {(char *)LINE16}};
...
SelectMessage Show[16] = {{(char *)SAVESET}, {(char *)WAVCAL_ON}, {(char *)WAVCAL_OFF}, {(char *)STDBYTIM_ON}, {(char *)STDBYTIM_OFF}, {(char *)CURMETERS_ON}, {(char *)CURMETERS_OFF}, {(char *)TRGHLD_ON}, {(char *)TRGHLD_OFF}, {(char *)SHOWEDGE_ON}, {(char *)SHOWEDGE_OFF}, {(char *)WAIT_RESET}, {(char *)NOR_DISPLAY}, {(char *)RAW_DATA}, {(char *)CUR_DIS_ON}, {(char *)CUR_DIS_OFF}};
...
int main(void)
{
  ...
  __Display_Str(10, 50, CYAN, PRN, APP_VERSION);
  __Display_Str(100, 130, ORANGE, PRN, APP_REVISION);
  __Display_Str(104, 30, ORANGE, PRN, "Costomized by M.nohira");
  Y_Attr = (Y_attr *)__Get(VERTICAL);
  X_Attr = (X_attr *)__Get(HORIZONTAL);
  G_Attr = (G_attr *)__Get(GLOBAL);
  T_Attr = (T_attr *)__Get(TRIGGER);

  if (memcmp((u8 *)__Get(13), "2.81", 4) == 0)
    HardwareVersion = 1;
  else
    HardwareVersion = 0; //HDWVER
  ...
  if (i == 0) // read the default boot parameters
    __Display_Str(125, 10, BLUE, PRN, "Configuration OK");
  else
    __Display_Str(80, 30, RED, PRN, "Configuration file not found");
  ...
  if ((u16)__Get(0) == 0x5731)
    FPGAver = 1;    //get FPGA version
  __Set(32 + 5, 1); //load data
  __Set(32 + 5, 1); //register
  if ((u16)__Get(0) == 0x0101)
    FPGAsubVer = 1;        //get FPGA update version
  GPIOC->BRR = 0x00000010; //Reset port C bit 4

  if (FPGAver > 0)
    StartOffset = 14;
  else
    StartOffset = 16; //for oversampling buffer function
  ...
  while (1)
  {
    ...
    //-------------------------------------------------------------------------- ********* LIFE *************
    if (((__Get(USB_POWER) > 0) || (Timeout == 0)) && (PD_Cnt > 0))
      PD_Cnt = 600; //don't time out into standby on external power or if timeout is disabled and not already in standby
    if (PD_Cnt == 0)
    {
    ...
    else
    {
      ...
      if (SelectiveUpdate & 1)
      { // restore screen after removing chart
        if (_Mode == X_Y)
        {
      ...
      if ((TrgAuto > 0) && (_Mode != SCAN) && (_Mode != AUTO) && (_Mode != SGL))
      {
        ...
        { //check to see if not triggered w/auto trig on
          loopcount++;
          if (loopcount > TrigReset[_T_base])
          { //after proper wait time, re-initialize trig
            if (Title[TRIGG][SOURCE].Value == 4)
            { //in A&B mode, reset only non triggered ch
              ...
              {
                if (TrigSourceEnable == 0)
                  _Vt2 = _2_posi; //alternate channel function call at end of synchro has shifted over to
                if (TrigSourceEnable == 1)
                  _Vt1 = _1_posi; //other ch if one ch is not triggered so need to reset alternate ch
              }
              Update_Trig(0);
            }
            else
        ...
        { //if triggered
          ...
          { //in A&B mode
            if (TrigSourceEnable == 0)
              ABTrigStatus |= 1; //ch A is triggered
            if (TrigSourceEnable == 1)
              ABTrigStatus |= 2; //ch B is triggered
            if (ABTrigStatus == 3)
            {
              loopcount = 0;
              ABTrigStatus = 0;
            } //if both ch are triggered, reset count and trigger status
          }
          else
            loopcount = 0;
      ...
      if (Second != Sec_Cnt)
      {
        ...
        if ((__Get(V_BATTERY) > (PrevBatLevel + 40)) || (__Get(V_BATTERY) < (PrevBatLevel - 40)))
        { //update with greater than 40Mv change (WAS +/-20Mv)
        ...
        if (FlagMeter == 1)
        { // refresh the measured values ??per second
          ...
          for (i = 0; i < 9; ++i)
            Display_Value(i);
          if (((__Get(FIFO_START) != 0) || ((_Mode != NORH) && (_Mode != NORHLD))) || ((DownConvertInitiate == 1) && (FFTt1Mode)))
          { //hold reset to show meter values when in normal mode not triggered
            ...
            WaitForTrigFlag = 1; //allows reset to occur prior to meter loads on triggering after holding values
        }

        if (((FlagMeter == 0) && (EnableMeterCalc == 0)) &&
            ((DownConvertMode == 0) || (DownConvertInitiate == 1)))
          ResetSum(); //DownConvert initiates here
      }

      if ((ChartLogic()) && (CursorDisplaySelect < 4))
        CalculateTvernier(1); //Xpos/Tvernier display moving update

      //============================================================================================================================

      if (((Cnt_20mS % 16) > 7) && ((Cnt_20mS % 16) < 16) && (UpdateFlag == 1))
        UpdateFlag = 0;

      //===========================================LARGE METER TIMING===========================================================
      if (((Cnt_20mS % 16) >= 0) && ((Cnt_20mS % 16) < 8) && (UpdateFlag == 0))
      {                 //update meters %16= 3 times/sec %24= 2 times/sec  %12= 4 times/sec
        ...
        if (_Kind == PWM)
          LimitTransfer = ((D_Tab[_Frqn].PSC + 1) * D_Tab[_Frqn].ARR) / 2; //bailout calculations for sweep wave ending functions
        else
          LimitTransfer = A_Tab[_Frqn] * (TIM7->PSC + 1) * 125;

        UpdateBigMeters();

        if ((FlagMeter != 1) && (EnableMeterCalc))
        { //allow to get frequency for detector mode display with meters off
          DETflag = 1;
          Display_Value(FRQ);
          DETflag = 0;
        }
        ...
        if (((FlagMeter == 2) || ((FlagMeter != 1) && (EnableMeterCalc))) || ((DownConvertMode) && ((DownConvertInitiate != 1) || (FFTt1Mode))))
        {
          if (((__Get(FIFO_START) != 0) || ((_Mode != NORH) && (_Mode != NORHLD))) || ((DownConvertMode) && (FFTt1Mode)))
          {
            if ((_T_base > 10) || ((_T_base > 8) && (FastMode == 1)))
            {
              if ((DownConvertMode) && (DownConvertInitiate == 0))
                Display_Value(0);
              ResetSum();
            }
            else
            {
              ...
              if ((__Get(FIFO_START) == 0) && (DownConvertMode) //allows FFT1 mode to work without a triggered signal
                  && (DownConvertInitiate == 0) && (FFTt1Mode))
                Display_Value(0);
            }
            WaitForTrigFlag = 0;
          }
      ...
      Display_Title();
      //1-Twink, INV, OR PRN
      if ((Current != FILE) && (NotificationTimer > 4))
        Print_Str(95, 0, 0x0405, PRN, Show[MessageNumber].Message); //show message
      if ((Current != FILE) && (NotificationTimer > 0) && (NotificationTimer < 5))
      {
        Update_View_Area();
        NotificationTimer = 0;
      } //end after delay
      ...
      if (Update)
      { // handle button to refresh item
        if ((DisableCursorTimer) && (Update == 2))
          DisableCursorTimer++;
        UpdateCursorMeter();

        if ((SelectiveUpdate & 1) && (FlagMeter > 0))
          EnableMeter();
        for (i = 0; i < 4; i++)
        {
          if (TrgAuto == i)
            Print_Str(365, 216, ((SCRN << 8) + Title[TRIGG][SOURCE].Value), i, (char *)AutoStr[i]);
        }
        if ((SelectiveUpdate & 0x02) == 0)
          Update_Range(); //these 4 are exclusions, bit set prevents update
        if ((SelectiveUpdate & 0x04) == 0)
          Update_Base();
        if ((SelectiveUpdate & 0x08) == 0)
          Update_Output();
        if ((SelectiveUpdate & 0x10) == 0)
        {
          if (Title[TRIGG][SOURCE].Value == 4)
            Update_Trig(0);
          else
        ...
        if (SelectiveUpdate & 0x20)
        {
          ...
          if (_T_base < 11)
            SlowModeSkip = 2; //slowmodeskip allows x frames to finish to properly update buffer
        }
        ...
        {
          if ((Title[0][SOURCE].Value == 2) || (Title[0][SOURCE].Value == 4))
            InvertA = 1;
          if ((Title[1][SOURCE].Value == 2) || (Title[1][SOURCE].Value == 4))
            InvertB = 1;
        }

        if ((Tmp != InvertA) || (TmpVT != InvertB))
        {
          if (Title[TRIGG][SOURCE].Value == 4)
            Update_Trig(0);
          else
        ...
        if (Current < 4)
          Title[Current][3].Flag |= UPDAT;
        if ((Current == TRIGG) && (_Det == 3))
        {
          if ((FrameMode == 0) || ((Options & 1) == 0) || (_Mode == AUTO) || (OsBufferLogicFlag) || (_Mode == X_Y) || (_Mode == SGL) || (_Mode == SCAN) || (_T_base > 9))
            _Det = 0;
          Title[Current][_Det].Flag |= UPDAT;
        }

        if ((Current != FILE) && (NotificationTimer < 43))
          Update_View_Area(); //after 3/4 second, allow notification overwrite if button pushed
        if (FlagMeter > 0)
        {
        ...
        if (FrameMode == 2)
          SaveShortBuffXpos = XposRef; //save trig cursor in config file
        if (Update)
        ...
        if ((Current == FILE) && (_Det == DIR))
        {
          ...
          {
            for (i = 0; i < 3; i++)
              Ext[i] = F_EXT[Title[9][2].Value][i + 1];
          }
          ...
          ReadDir((char *)Ext);
          if (Label[Title[9][3].Value][0] == 0)
          {
            if (Title[9][3].Value > 0)
              Title[9][3].Value--;
          } //if presently selected last file non existant
          if (Label[0][0] == 0)
            for (i = 0; i < 13; i++)
              Label[0][i] = NoneStr[i]; //no files found
          if (DirRange > 0)
          {
            if (Label[14][0] == 0)
              for (i = 0; i < 13; i++)
                Label[14][i] = NoneStr[i];
          } //no more files found
          PrintDir(0, 0, 2);
        }
        if ((BufferRestore) && (Title[RUNNING][STATE].Value == 0))
        {
          UpdateFileMenu();
          BufferRestore = 0;
        }
      ...
      if ((FlagMeter == 0) && (ClearMeterAreaFlag))
      {
        Clear_Meter_Area();
        Update = 1;
      }
      ClearMeterAreaFlag = 0;

      if (_T_base > 10)
      {
        if (UpdateBackground == 3)
          UpdateBackground = 0;
    ...
    if ((TrgAuto > 0) && ((_Mode != SCAN) || (AutoSetFlag)))
    {
      ...
      {
        if (((_Mode == NORH) || (_Mode == NORHLD) || (_Mode == NORC)) && (_T_base < 10) && (Options & 1) && (!OsBufferLogicFlag))
          _Det = 3;
        else
          _Det = 0;
        Title[Current][_Det].Flag |= UPDAT;
    ...
    if (Update == 1)
      SelectiveUpdate |= 0x08; //anything setting update up to this point has nothing to do with gen output

    if (RefreshDisplay)
    {                   //update display when changing buffer size while not triggered un uart mode
    ...
    if (Key_Buffer)
    {
      ...
      for (i = 0; i < 4; i++)
      { //clear memorized trigger settings of ch if ch is turned off
      ...
      if (Key_Buffer == KEY1)
      {
        ...
        while (Delay_Cnt > 1000)
        {
          if ((__Get(KEY_STATUS) & KEY1_STATUS) != 0)
          {
            ...
            if ((_State.Value == 0) && (AutoSetFlag == 0))
            {                //0=run 1=hold also has 2
            ...
            else
            {
              ...
              else
              {
                ...
                if (_Mode != NORH)
                  ...
                  ClearLeadingEdge = 1; //REMOVING NORH CONDITION WOULD CLEAR WAVE ALONG WITH METERS...
                if (((_Mode == SGL) || (_Mode == NORH) || (Options & 4) || ((FrameMode > 0) && (_T_base < 3)) || ((FrameMode == 0) && (_T_base < 6))) && (_Mode != NORHLD))
                { //in run mode if frame >1 1/2 sec
                  if (!(ChartLogic()))
                  {
        ...
        Beeper(125);
        BackLight(1);
        while ((Delay_Cnt > 0) && (Delay_Cnt <= 1000))
        { // long press
          if (Delay_Cnt < 875)
            BackLight(0);
          if ((__Get(KEY_STATUS) & KEY1_STATUS) != 0)
          {
            ...
            case TRACK1:
              ...
              {
                if (Title[TRACK4][SOURCE].Value < 10)
                  EnableChannel(TRACK1); //if FFT on, do not turn channel trace on
              }
              break;
            case TRACK2:
              if (_Det == RANGE)
              ...
              break;
            default:
              if ((Title[TRACK1][SOURCE].Value != HIDE) && (Title[TRACK2][SOURCE].Value == HIDE))
              { //Cha A on B off
                Current = TRACK1;
                LastSelected = 1;
              }
              else if ((Title[TRACK1][SOURCE].Value == HIDE) && (Title[TRACK2][SOURCE].Value != HIDE))
              { //Cha B on A off
                Current = TRACK2;
                LastSelected = 2;
              }
              else if (_4_source < 10)
              { //if FFT off												//both on or off
                switch (LastSelected)
                {
                case 0:
                case 1:
              ...
              { //FFT on, both tracks on or both off
                if (_4_source < 14)
                  Current = TRACK2;
                else
                  Current = TRACK1; //go to ch v/div but do not turn on
              }
            } //switch
            _Det = RANGE;
      ...
      {                   // ===--- TASTO 2 PREMUTO ---===
        ...
        while (Delay_Cnt > 2500)
        { // Se il tempo non � arrivato a 0
          ...
          { // CAMBIO MODO TRIGGER
            ...
            if ((Title[TRIGG][SOURCE].Value != TRACK3) && (Title[TRIGG][SOURCE].Value != TRACK4))
            {
              ...
              if ((Sweep) && (_Tr_kind == 8) && (TrgAuto == 2) && (_Kind < 5))
                TrgAuto++; //auto trig= 1/2 does not work well with kind=GEN
              if (TrgAuto > 3)
              {
                TrgAuto = 1;
              } // short press
        ...
        while ((Delay_Cnt > 0) && (Delay_Cnt <= 2500))
        { // long press
          if (Delay_Cnt < 2375)
            BackLight(0);
          if ((__Get(KEY_STATUS) & KEY2_STATUS) != 0)
        ...
        if (Delay_Cnt == 0)
        {
          ...
          else
          {
            ...
            if ((Current == T_BASE) && (_Det == 1) && (OSBufferLogic()) && (FPGAsubVer) && (FPGAosFlag))
            { //Tbase RANGE sub menu
              if (OS_RateSelect)
                OS_RateSelect = 0;
              else
                OS_RateSelect = 1;
      ...
      { // ===--- TASTO 3 PREMUTO --====
        ...
        if ((Current != FILE) || (_Curr[2].Value != CFG))
        {
          ...
          while (Delay_Cnt > 1000)
          { //WAS >0           Se il tempo non � arrivato a 0
            if ((__Get(KEY_STATUS) & KEY3_STATUS) != 0)
            {
              ...
              if (DownConvertMode == 0)
              {
                if (FlagMeter == 2) //if in big meters, shut off
                {
                  if (_Mode != X_Y)
                  {
                    if ((ListLogic()) && (EditListActive(1)))
                      PrintDir(0, 0, 3);
                    DisableMeter();
                    if ((_4_source == SPEC_A) || (_4_source == SPEC_B) || (_Mode == NORHLD))
                      Clear_Meter_Area();
                  }
                  else
                  { //toggle the 2 meters in xy mode, instead of shutting meters off
                    FlagMeter = 1;
                    EnableMeter();
                  }
                  goto bypasslongpress;
                ...
                {
                  if (FlagMeter == 1)
                  { //if in small meters go to big meters
                    if (Current > METER_3)
                      Current = METER_3;
                    if (UpdateMeter == 4)
                    {
                    ...
                    else
                    {
                      ...
                      if ((Current < METER_0) && (Current > T_VERNIE))
                        Current = T_VERNIE; //if not in meter item select mode
                    }
                    FlagMeter = 2;
                  }
                  if ((FlagMeter == 0) && (ListLogic()) && (EditListActive(1)))
                    PrintDir(0, 0, 3);
          ...
          { // long press
            ...
            if ((__Get(KEY_STATUS) & KEY3_STATUS) != 0)
            {
              ...
              if (Title[OUTPUT][0].Value != 6)
              { //if generator is not turned off
          ...
          if (_Curr[2].Value == CFG)
          {
            if (_Curr[0].Value == SAVE)
            {
              if (_Det == 3)
              {
                ...
                ProcessFileName();
                for (i = 0; i < 8; i++)
                  LastAccessedConfig[i] = SelectedFileName[i];
            ...
            else
            {
              ...
              {
                for (j = 0; j < 8; j++)
                  LastAccessedConfig[j] = SelectedFileName[j] = Label[Title[9][3].Value][j];
              }
              j = Title[FILE][3].Value; //keep present hilighted list position
              i = ConfigFile(LOAD);
              if (ListOverride)
              { //keep list after loading new config from it
                Title[FILE][3].Value = j;
                Detail[9] = 3;
                ProcessEditName(); //loads processed name into edit function
              }
              for (j = 0; j < 3; j++)
                Title[FILE][j].Flag |= UPDAT;
            }
          }
        }
        ...
        while (Delay_Cnt > 1000)
        {
          if ((__Get(KEY_STATUS) & KEY4_STATUS) != 0)
          {
            ...
            if ((Current == FILE) && (_Det == DIR))
            {
              ...
              if ((_Mode == X_Y) && (Title[9][0].Value == 1))
                InitXY = 1;
            }
            ...
            if ((Current == OUTPUT) && (_Kind == 6))
            {
              _Det = 0;
              goto BypassUpdate;
            }

            if ((Current == TRACK3) && (_Det == 0))
              _Det = 2;
            ...
            if ((Current == TRACK4) && (_Det == 0))
            {
              if (_4_source > 11)
                ...
                _Det = 2; //in fft modes, to engage MAG modes
              if (_4_source > 10)
                goto bypasslongpress; //in fft modes, 11=spec
            }
            else if ((Current == TRACK4) && (_Det == 2) && (_4_source > 9))
            { //in map mode
              _Det = 0;
              goto bypasslongpress;
            }
            ...
            if (Current == 6)
            {
              ...
              else if ((_Det == 2) && (_Mode != NORHLD))
                _Det = 3;
            }

            if ((((Options & 1) == 0) || (_Mode == AUTO) || (_Mode == X_Y) || (OsBufferLogicFlag) || (_Mode == SGL) ||
                 (_Mode == SCAN) || (_T_base > 9)) &&
                (Current == TRIGG) && (_Det == 2))
              _Det = 3; //skip over det3 (will go to 0)

            if ((FlagMeter == 0) && ((Current == V_VERNIE) || (Current == T_VERNIE)) && (_Det == V2))
            { //skip meter select if meters off
              _Curr[_Det].Flag |= UPDAT;
            ...
            else
            {
              ...
              { // change the Detail
                ...
                if (Current == OUTPUT)
                {
                  ...
                  else if (_Det == DUTYPWM)
                    _Det += 1;
                }
                if (_Curr[_Det].MARK & NOT)
                  _Det = 0;
                if (_Curr[_Det].MARK & NUM2)
                  _Det = 0;
                if ((_Curr[_Det].MARK & NUM3) && (Current != FILE) && (Current != OUTPUT))
                  _Det = 0;

                if ((Current == FILE) && (_Det == 3))
                {
                  if ((_Curr[2].Value == 7) && (Title[FILE][0].Value == 0))
                    _Det = 0; //do not edit ROM backup file
                }

                _Curr[_Det].Flag |= BLINK;
              }
              else
              { // change the measurement object

                if (Meter[Current - METER_0].Item > 1)
                { //don't change source for VBT AND FPS (items 0 and 1)
      ...
      { // ===--- TASTO 5 --- PREMUTO --====
        ...
        if ((((_Mode == SCAN) && (!ChartLogic())) || ((ChartLogic()) && (FrameMode > 0))) && (Current == T_BASE) && (_Det == 2))
          goto BypassUpdate; //bypass changing Xpos in chart mode short buffers
        ...
        if ((_4_source > 11) && (Current == 3) && (_Det == 1))
        {
          if (DownConvertMode)
          {
            if (DownConvert == 5)
              DownConvert = 2;
            else if (DownConvert == 50)
              DownConvert = 20;
            else if (DownConvert == 500)
              DownConvert = 200;
            else if (DownConvert > 1)
              DownConvert /= 2;
          ...
          else if (FFTt1Mode)
          {
            if (_T1 > 0)
              _T1--;
            if (_T1 > 2)
              _T1 -= CursorSpeedLogic(1);
          }
          goto BypassUpdate;
        }

        if ((DownConvertMode) && (Current == T_BASE) && (_Det == BASE))
          goto BypassUpdate;
        ...
        { //filename edit
          if ((CharValue > 65) && (CharValue < 91))
            CharValue--; //letters
          else if ((CharValue > 47) && (CharValue < 57))
            CharValue++;
          else
            switch (CharValue)
            {
            ...
            case 63:
              CharValue = 48;
              break; //63= non available char flag
            case 57:
              CharValue = 95;
              break;
            case 95:
        ...
        if (_Mode == X_Y)
        {
          if ((Current == TRACK1) && (_Det == POSI) && (OffsetX > 45))
            OffsetX = OffsetX - 2;
          if ((Current == TRACK2) && (_Det == POSI) && (OffsetY > 30))
            OffsetY = OffsetY - 2;
        }
        SelUpdate(POSI); //clear min/max hold

        if (((Current == TRACK1) || (Current == TRACK2)) && (_Det == 1))
          AlignTbaseSweep(0); //changing coupling mode

        if ((Current == 5) && (_Det == 1) && (_Kind == 8))
        { //uart gen mode
          if (GenUartAdj == 0)
          { //adjust baud index, translate to baud rate
            if (GenBaudAdjSpeed)
            { //fast mode
              if (GenBaudIndex < 800)
              { //set up break points for fast adj mode
                GenBaudIndex += 20;
              }
        ...
        if ((T1Start) && (Current == T_VERNIE) && ((_Det == T1) || (_Det == T2)) && (Title[TRACK1][COUPLE].Value > 2) && (Title[TRACK1][COUPLE].Value < 5))
        { //in serial decode, tie T1 to T2
          if ((_T1 > 0) && (_T2 > 0))
          {
            _T1--;
            _T2--;
        ...
        if ((Current == OUTPUT) && (_Det == 0) && (_Kind == 0))
        { //clear buffer before changing to Arbt
          for (i = 0; i < 720; i++)
            ATT_DATA[i] = 2048;
        }

        if ((Current == OUTPUT) && (_Det == 1) && (GenAdjustMode == 1) && ((_Kind < 5) || (_Kind == 7)))
        { //changing sweep or burst rate
        ...
        if (Current < METER_0)
        {
          ...
          for (i = 0; i < 4; i++)
          {
            ...
            { //ch selected and submenu on source
              if (Title[i][SOURCE].Value == 1)
              { //about to be shut down
                if (Title[TRIGG][SOURCE].Value == i)
                { //if trig selected
                  UpdateTriggerMem();
                  TriggFlagUpdate();
                  Update_Trig(1);
          ...
          if ((Current == TRIGG) && (Detail[Current] == 2))
          { // trigger level adjustment
            ...
            for (i = 0; i < j; i++)
            {
              if (Title[TRIGG][SOURCE].Value == 4)
              {
                if (V_Trigg[0].Value > 0)
                  V_Trigg[0].Value--;
                if (V_Trigg[1].Value > 0)
                  V_Trigg[1].Value--;
              }
              else
              {
          ...
          else if ((Current == TRIGG) && (_Det == 3))
          { //trigger holdoff
            if ((FrameMode == 2) && (_T_base < 10))
            {
              if (Title[7][3].Value > 30)
                Title[7][3].Value -= 30;
              else
                Title[7][3].Value = 0;
          ...
          { //  adjust backlight or volume
            if (_Curr[1].Value > 0)
              _Curr[1].Value--;
          }
          else if ((Current == T_BASE) && (_Det == XPOSI))
          {                // X_POSI adjustment
            ClearMeters(); //clears meters if untriggered in NORH
          ...
          { // PWM duty% adjust
            ...
            {
              if (_Curr[_Det].Value > 300)
                _Curr[_Det].Value -= 300;
              else
                _Curr[_Det].Value = 0;
            }
          }
          else
          { //  for other current item
            if ((Current == TRACK1) && (_Det == SOURCE))
            {

              if (_1_source == 0)
              { //ch A about to be turned on, else off
                LastSelected = 1;
            ...
            if ((Current == TRACK2) && (_Det == SOURCE))
            {
              ...
              if (_2_source == 0)
              { //ch B about to be turned on, else off
                LastSelected = 2;
            ...
            {
              if (PerstFrameNumber > 0)
                PerstFrameNumber -= ((PerstFrameNumber / 65) + 1);
            }
            else
            {
              ...
              else
              {
                ...
                if (_Curr[_Det].Value > 0)
                {
                  _Curr[_Det].Value--;
                  if ((Current == 6) && (_Det == 3))
                    _Curr[_Det].Value -= (_Curr[_Det].Value / 65); //establish 3 breakpoints for speed change
                  SelUpdate(RANGE);
                  if ((Current < 2) && (_Det == RANGE))
                    Update_Range();
                }
                else if (_Curr[_Det].MARK & CIRC)
                  _Curr[_Det].Value = _Curr[_Det].Limit;

                if (((Current == 0) || (Current == 1)) && (_Det == 0) && (_Mode == X_Y))
                {
                  if (Title[Current][0].Value > 0)
                    Title[Current][0].Value--;
                }
              }
            }

            if ((Current == TRIGG) && (Detail[Current] == 0))
            { //trig source has just been changed
              if (Title[TRIGG][SOURCE].Value == 4)
              { //changed to AltAB
                if (_Mode == X_Y)
                {
            ...
            if ((Current == T_BASE) && (_Det == BASE))
            {
              ...
              if (!OSBufferLogic())
              {
                if ((__Get(FIFO_START) != 0) || ((_Mode != NORH) && (_Mode != NORHLD)))
                  __Set(TRIGG_MODE, UNCONDITION);
              } //temporarily setting this allows x_attr to properly update if in untriggered condition
            }
            if ((FrameMode < 2) && (Current == T_BASE) && (_Det == BASE))
            {
              Tmp = GetXposRef(_X_posi.Value);
              if (Tmp > 150)
                XposRef = Tmp; //only align trig vernier when on screen when changing time bases
            }
          }
          ...
          if ((Current == OUTPUT) && (_Det == 1))
            PWMscaleReset(1);

          if ((Current == T_BASE) && (_Det == MODE))
          { // T_BASE MODE selection			//at this point mode has been changed
            if (_Mode == SCAN)
            {
              if (FrameMode > 0)
          ...
          { //changing file extensions
            ...
            if (_Curr[0].Value == 0)
            { //SAVE (edit filename)
              if ((_Curr[2].Value == 5) || (_Curr[2].Value == 6))
              {
                _Curr[2].Value = 4;
                ResetEditList();
              }
          ...
          if ((Current == FILE) && (_Curr[2].Value == CFG))
            if (_Curr[1].Value > 9)
              _Curr[1].Value = 9;
          if ((Current == FILE) && (_Det == 2))
            ResetEditList(); //reset list and edit defaults when changing file types
          for (i = 0; i < 4; i++)
            _Curr[i].Flag |= UPDAT;
        }
        else
        {
          ...
          if ((Meter[Current - METER_0].Track > 1) && (Meter[Current - METER_0].Track < 4))
          {
            if (Meter[Current - METER_0].Item == VPP)
              Meter[Current - METER_0].Item = FRQ;
            if (Meter[Current - METER_0].Item == RMS)
              Meter[Current - METER_0].Item = FPS;
          }
        ...
        if (_Curr[_Det].Value > 2)
          _Curr[_Det].Value -= (CursorSpeedLogic(0));
      }
      //---------------------------------------------------------------------------------------------------------------------
      ...
      { // ===--- TASTO 5 centrale PREMUTO --====
        ...
        if ((Current != FILE))
        {
          ...
          while (Delay_Cnt > 1000)
          {
            ...
            if ((__Get(KEY_STATUS) & K_INDEX_S_STATUS) != 0)
            {
              ...
              if ((Current == TRIGG) && (_Det == 2))
                DisableCursorTimer = 0;
              //SPECIAL FUNCTIONS FOR LEFT TOGGLE CENTER PRESS
              if (LeftToggleSpecialFunctions())
                goto bypasslongpress;
              ...
              if (j > 0)
              {
                if ((((_4_source == 11) || (_4_source > 13)) && (_1_source != HIDE)) ||
                    (((_4_source == 10) || (_4_source == 12) || (_4_source == 13)) && (_2_source != HIDE)))
                  j--;
              }

              if (j > 2)
              { //if more than 2 active channels, use as shortcut to trigg source select
                _Curr[_Det].Flag |= UPDAT;
                for (i = 0; i < 4; i++)
                {
                  if (Current == i)
                  { //================================================================
                    if (Title[i][SOURCE].Value != HIDE)
                    {
                      ...
                      { //if not already selected
                        for (j = 0; j < 3; j++)
                          PreviousTrigSelect[j] = PreviousTrigSelect[(j + 1)]; //lifo stack
                        PreviousTrigSelect[3] = Title[TRIGG][SOURCE].Value;    //save previous
                        Title[TRIGG][SOURCE].Value = i;                        //set trig to selected track
                        if ((i == 3) && ((_4_source == 11) || (_4_source > 13)))
                          Title[TRIGG][SOURCE].Value = 0; //set to FFT channels
                        if ((i == 3) && ((_4_source == 10) || (_4_source == 12) || (_4_source == 13)))
                          Title[TRIGG][SOURCE].Value = 1;
                        Update_Trig(1);
              ...
              { //cycle between any other 2 channels
                for (i = 0; i < 4; i++)
                {
                  if (Title[i][SOURCE].Value != HIDE)
                  {
          ...
          { // long press, change meter pages
            ...
            if ((__Get(KEY_STATUS) & K_INDEX_S_STATUS) != 0)
            {
              ...
              if (FlagMeter > 0)
              {
                ...
                if (UpdateMeter < 4)
                {
                  if ((FlagMeter == 2) && (UpdateMeter == 3) && (Current == METER_1))
                    Current = METER_2;
                  UpdateMeter = UpdateMeter + 1;
                  if ((_Mode == X_Y) && (UpdateMeter > 1))
                    UpdateMeter = 4;
                }
                else
                { //UpdateMeter==4, default pages for both meter types
                  if (FlagMeter == 2)
                  {
                    ClearScreenArea(314, 399, 125, 180);
                    if ((Current < METER_0) && (Current > T_VERNIE))
                      Current = T_VERNIE;
                    if ((Current == METER_0) || (Current == METER_2))
                      Current = METER_1;
                  }
                  UpdateMeter = 0;
                }
              ...
              { // flagmeter=0					//toggle wave calibration
        ...
        if (Current == FILE)
        {
          ...
          reload:
            ...
            {
              for (i = 0; i < 8; i++)
                SelectedFileName[i] = Label[Title[9][3].Value][i];
            }

            i = 0;
            if (_Curr[2].Value == DAT)
              i = Load_Dat(_Curr[1].Value);
            ...
            if (_Curr[2].Value == BMP)
            {
              ...
              if ((Current == FILE) && (_Det == DIR))
              {
                if (i == 0)
                  for (j = 0; j < 8; j++)
                    EditChar[j][0] = SelectedFileName[j];
                if (Title[9][3].Value < 14)
                  Title[9][3].Value++;
                else
                  DirRange++;
                ...
                while (1)
                {
                  ...
                  if (Key_Buffer > 0)
                  {
                    ...
                    if (Key_Buffer == K_ITEM_S)
                    {
                      ...
                      if ((Current == FILE) && (_Det == DIR))
                      { //use block to allow prev BMP load in list
                        ...
                        if (Title[9][3].Value > 0)
                        {
                          ...
                          for (j = 0; j < 8; j++)
                          {
                            if (SelectedFileName[j] != Label[Title[9][3].Value][j])
                              break;
                          }
                          if (j > 6)
                          {
            ...
            if (_Curr[2].Value == CFG)
            {
              ...
              if (ListOverride)
              { //keep list after loading new config
                Title[FILE][3].Value = j;
                for (j = 0; j < 8; j++)
                  LastAccessedConfig[j] = SelectedFileName[j];
                _Det = 3;
                ProcessEditName(); //loads processed name into edit function
              }
              for (j = 0; j < 3; j++)
            ...
            if (_Curr[2].Value == CSV)
            { //load CSV file not available
              i = 200;
            }

            if (_Curr[2].Value == 7)
            { //load bin image not available
              i = 200;
            }

            if ((_Det == DIR) && (_Curr[2].Value < 3))
            { //use last loaded filename as edit default
              if (i == 0)
                for (j = 0; j < 8; j++)
                  EditChar[j][0] = SelectedFileName[j];
            }
      ...
      { // ===--- TASTO 5 +++ PREMUTO --====
        ...
        if ((_4_source > 11) && (Current == 3) && (_Det == 1))
        {
          if (DownConvertMode)
          {
            if (DownConvert == 2)
              DownConvert = 5;
            else if (DownConvert == 20)
              DownConvert = 50;
            else if (DownConvert == 200)
              DownConvert = 500;
            else if (DownConvert < 1010)
              DownConvert *= 2;
            DownConvertShiftEnable = 1;
            goto BypassUpdate;
          }
        ...
        if (_Mode == X_Y)
        {
          if ((Current == TRACK1) && (_Det == POSI) && (OffsetX < 215))
            OffsetX = OffsetX + 2;
          if ((Current == TRACK2) && (_Det == POSI) && (OffsetY < 170))
            OffsetY = OffsetY + 2;
        }
        SelUpdate(POSI);

        if (((Current == TRACK1) || (Current == TRACK2)) && (_Det == 1))
          AlignTbaseSweep(1); //changing coupling mode
        ...
        if ((T1Start) && (Current == T_VERNIE) && ((_Det == T1) || (_Det == T2)) && (Title[TRACK1][COUPLE].Value > 2) && (Title[TRACK1][COUPLE].Value < 5))
        { //in serial decode, tie T1 to T2
          if ((_T1 < Title[T_VERNIE][T1].Limit) && (_T2 < Title[T_VERNIE][T2].Limit))
          {
            _T1++;
            _T2++;
          }
        ...
        if (Current < METER_0)
        {
          ...
          { //tracks 1 through 4
            if ((Current == i) && (_Det == SOURCE))
            {
              ...
              { //if trig selected
                ...
                switch (Current)
                { //ch about to be shut down
                case TRACK1:
                case TRACK2:
                  //if (Title[i][SOURCE].Value == 2)UpdateTriggerMem();
                  if (Title[i][SOURCE].Value == 4)
                    UpdateTriggerMem();
                  break;
                case TRACK3:
                  if (Title[i][SOURCE].Value == 1)
          ...
          { // trigger level adjustment
            ...
            for (i = 0; i < j; i++)
            {
              if (Title[TRIGG][SOURCE].Value == 4)
              {
                if (V_Trigg[0].Value < 200)
                  V_Trigg[0].Value++;
                if (V_Trigg[1].Value < 200)
                  V_Trigg[1].Value++;
              }
              else
              {
          ...
          { // adjust backlight or volume
            if (_Curr[1].Value < _Curr[1].Limit)
              _Curr[1].Value++;
          }
          else if ((Current == T_BASE) && (_Det == XPOSI))
          {                // *******>>>>>> ALT X_POSI adjustment
            ClearMeters(); //clears meters if untriggered in NORH
            ...
            if (XposRef < 151)
            {
              ...
              else if (_Curr[_Det].Value < (_Curr[_Det].Limit))
                _Curr[_Det].Value++;
            }
            else
            {
              if (SetLowVolume)
              {
                ...
                else if (_Curr[_Det].Value < (_Curr[_Det].Limit - 6))
                  _Curr[_Det].Value += 6;
                else if (_Curr[_Det].Value < (_Curr[_Det].Limit))
                  _Curr[_Det].Value++;
              }
          ...
          else if ((Current == T_VERNIE) && ((_Det == T1) || (_Det == T2)))
          {
            if (FlagMeter > 0)
            {
              if (_Curr[_Det].Value < (_Curr[_Det].Limit - 95))
                _Curr[_Det].Value++;
              if (_Curr[_Det].Value < (_Curr[_Det].Limit - 97))
                _Curr[_Det].Value += (CursorSpeedLogic(0));
            }
            else
            {
              if (_Curr[_Det].Value < _Curr[_Det].Limit - (MIN_X - 2))
                _Curr[_Det].Value++;
              if (_Curr[_Det].Value < (_Curr[_Det].Limit - MIN_X))
                _Curr[_Det].Value += (CursorSpeedLogic(0));
            }
          }
          else if ((Current == OUTPUT) && (_Det == 2))
          { // PWM duty% adjust
            ...
            {
              if (_Curr[_Det].Value < 29399)
                _Curr[_Det].Value += 300;
              else
                _Curr[_Det].Value = 29999;
            }
          }
          else
          { // the current item for otheR

            if (_Curr[_Det].Value < (_Curr[_Det].Limit - 2))
              _Curr[_Det].Value += (CursorSpeedLogic(0));
            ...
            else
            {
              ...
              else
              {
                ...
                if (_Curr[_Det].Value < _Curr[_Det].Limit)
                {
                  _Curr[_Det].Value++;
                  if ((Current == 6) && (_Det == 3))
                    _Curr[_Det].Value += (_Curr[_Det].Value / 65);
                  SelUpdate(RANGE);
                  if ((Current < 2) && (_Det == RANGE))
                    Update_Range();
                }
                ...
                if (((Current == 0) || (Current == 1)) && (_Det == 0) && (_Mode == X_Y))
                {
                  if (Title[Current][0].Value > 0)
                    Title[Current][0].Value++;
                  if (Title[Current][0].Value > 4)
                    Title[Current][0].Value = 0;
                }
              }
            }
            ...
            { //trig source has just been changed
              ...
              if ((Title[TRIGG][SOURCE].Value == 0) && (_Mode != X_Y))
              {                                      //changed from AltAB
                Title[T_BASE][MODE].Value = OldMode; //reset to prev mode
                Title[T_BASE][MODE].Flag |= UPDAT;
                ChannelStatus(1); //restore status
          ...
          if ((Current == FILE) && (_Curr[2].Value == CFG))
            if (_Curr[1].Value > 9)
              _Curr[1].Value = 9;
          if ((Current == FILE) && (_Det == 2))
            ResetEditList(); //reset file list hilight to top when changing file types
          for (i = 0; i < 4; i++)
            _Curr[i].Flag |= UPDAT;
        }
        else
        {
          ...
          if ((Meter[Current - METER_0].Track > 1) && (Meter[Current - METER_0].Track < 4))
          {
            if (Meter[Current - METER_0].Item == VDC)
              Meter[Current - METER_0].Item = FRQ;
            if (Meter[Current - METER_0].Item == MAX)
              Meter[Current - METER_0].Item = CIR;
          }
        }
      }
      //----------------------------------------------------------------------------------------------------------------
      ...
      { // ===--- TASTO 6 --- PREMUTO --====
        if (Current < METER_0)
        {
          ...
          { //filename edit
            if (CharIndex > 0)
            { //allow right toggle to escape edit box
              CharIndex--;
              CharValue = EditChar[CharIndex][0];
              UpdateEditBox(3);
              goto bypasslongpress;
          ...
          if ((Current == FILE) && (_Det == DIR) && (_Mode == X_Y) && (Title[9][0].Value == 1))
            InitXY = 1;
          ...
          if (Current == T_VERNIE)
          {
            Current = V_VERNIE;
            goto NoDec;
          } //skip over file menu's regular place
          ...
          if (Current == FILE)
          {
            ...
            else
            {
              if ((_Det == 3) && (ListLogic()))
                PrintDir(0, 0, 3); //clear file list when leaving
              if ((FlagMeter == 1) || ((FlagMeter == 2) && (UpdateMeter == 4)))
              {
                Current = VOLUME;
                goto NoDec;
              }
          ...
          if (Current < 4)
            Title[Current][3].Flag |= UPDAT;
          if (Current == RUNNING)
            Current--; // Jump over Item 4
        NoDec:
          CurDetailUpdate();
          if (Current == FILE)
          {
            if ((_Curr[2].Value == CFG) && (_Curr[1].Value > 9))
              _Curr[1].Value = 9;
            for (i = 0; i < 4; i++)
              _Curr[i].Flag |= UPDAT;
          }
      ...
      { // ===--- TASTO 6 centrale PREMUTO --====
        ...
        while (Delay_Cnt > 1000)
        {
          if ((__Get(KEY_STATUS) & K_ITEM_S_STATUS) != 0)
          {
            ...
            if ((Current == FILE) && (_Curr[2].Value == BUF) && (_Curr[0].Value == LOAD) && (_State.Value > 0))
            {
              if (_Det == 3)
              {
                if (Title[9][3].Value > 0)
                {
                  ...
                  for (i = 0; i < 8; i++)
                  {
                    if (SelectedFileName[i] != Label[Title[9][3].Value][i])
                      break;
                  }
                  if (i > 6)
                  {
            ...
            {
              if (Sweep == 0)
              { //shift from continuous to sweep mode, sweep rate adjust
              ...
              { //shift from sweep mode to burst mode, burst rate adjust
                GenFreqShift = PrevShift;
                Sweep = 2;
              }
              else if (Sweep == 2)
                Sweep = 3; //in burst mode, shift from burst rate to burst length adjust
              else if (Sweep == 3)
                Sweep = 4; //in burst mode, shift from burst length to freq adjust
              else if (Sweep == 4)
              {
                PrevShift = GenFreqShift;
                Sweep = 0;
              } //shift from burst mode, freq adjust to continuous mode
              TIM_2IRQControl();
              for (i = 0; i < 4; ++i)
                Title[OUTPUT][i].Flag |= UPDAT;
              if (Title[TRIGG][SOURCE].Value == 4)
                Update_Trig(0);
              else
                Update_Trig(1);
            ...
            else
            {
              ...
              {
                if (_Mode == X_Y)
                  goto bypasslongpress; //don't change buffer size this way in xy mode

                if (FlagFrameMode == 0)
                { //entering change from full buffer mode to single window
                  FlagFrameMode = 1;
                  SelectiveUpdate |= 0x60;
                  if (_Mode != SCAN)
                  {
                ...
                { //entering change from short buffers to full buffer mode
                  if (_State.Value > 0)
                    DigChLockout = 1; //when paused, don't display dig chs until reset, contains OS data at this point
                  OSBuffer = 0;
                  OSAvg = 0;
                  FPGAosFlag = 0;
                  FlagFrameMode = 0;
        ...
        { // long press
          ...
          if ((__Get(KEY_STATUS) & K_ITEM_S_STATUS) != 0)
          {
            ...
            if (EditListActive(0))
            { //reset all hilighted chars to space
              for (i = 0; i < 8; i++)
                EditChar[i][0] = 32;
              CharValue = 32;
              goto bypasslongpress;
            ...
            if ((Current == FILE) && (_Det == 3) && (_Curr[0].Value == LOAD))
            { //file delete

              for (i = 0; i < 13; i++)
                DelFileName[i] = Label[Title[9][3].Value][i];
              for (i = 0; i < 8; i++)
              {
                switch (DelFileName[i])
                {                      //process unavailable chars to display dot
                case 39:               //'
                case 64:               //@
                  DelFileName[i] = 63; //dot
                  break;
                default:
                  if (((DelFileName[i] > 32) && (DelFileName[i] < 37)) || (DelFileName[i] > 124))
                    DelFileName[i] = 63;
                }
              }
                ...
                if (Key_Buffer > 0)
                {
                  if (Key_Buffer == K_ITEM_S)
                  {
                    ...
                    if (i == 0)
                    {
                      ...
                      if (ListLogic())
                        ClearDir = 1; //clear file list when changing in SPEC or MAP modes
                    }
                    FileMessage(i);
                    break;
                  }
            ...
            {
              if (Sweep == 3)
              { //in burst mode, shift from burst length to burst rate adjust
                Sweep = 2;
                goto bypasslongpress;
              }
              else if (Sweep == 4)
      ...
      { // ===--- TASTO 6 +++ PREMUTO --====
        if (Current < METER_0)
        {
          ...
          {
            if (_Det == 3)
            { //scroll list down one page (limit to 14 to see last entry
          ...
          if (Current < 4)
            Title[Current][3].Flag |= UPDAT;
          if ((FlagMeter == 0) && (Current == V_VERNIE) && (_Det == 2))
            _Det = 0;
          if ((FlagMeter == 0) && (Current == T_VERNIE) && (_Det == 2))
            _Det = 0;

        NoInc:
          if (Current == RUNNING)
            Current++; // Jump over RUNNING
          CurDetailUpdate();

          if (Current == FILE)
          {
            if ((_Det == DIR) && (Title[9][0].Value == 0))
            { //filename edit
              CharValue = EditChar[CharIndex][0];
            }
            if ((_Curr[2].Value == CFG) && (_Curr[1].Value > 9))
        ...
        else if (FlagMeter == 2)
        {
          if (UpdateMeter != 4)
          {
            if (Current == METER_3)
              ...
              Current++;
          } //skip over meter 0 (bat volt), not displayed w/3 big meters
          else
          {
            if (Current == METER_0)
            {
    ...
    bypasslongpress:
      if ((Key_Buffer != K_INDEX_DEC) && (Key_Buffer != K_INDEX_INC))
      {
        OffsetSelect = 0;
        OS_RateSelect = 0;
      } //reset on all buttons except adjusts
    bypassOSselect:

      if (_4_source != SPEC_A)
        SpecRow = 25;
      MeterDefaults();
      if ((_1_source == 0) && (_2_source == 0) && (_3_source == 0) && (_4_source > 9))
      { //if FFT alone, set to proper ch
        if (_4_source > 13)
          Title[TRIGG][SOURCE].Value = 0;
        if ((_4_source == 13) || (_4_source == 12))
          Title[TRIGG][SOURCE].Value = 1;
        TriggFlagUpdate();
        Update_Trig(1);
      }
      else if (Title[TRIGG][SOURCE].Value == 4)
      {                                   //if trig source is a&b
        Title[T_BASE][MODE].Value = NORH; //set normal trig mode
        Title[T_BASE][MODE].Flag |= UPDAT;
        if (Title[TRACK1][SOURCE].Value == 0)
          Title[TRACK1][SOURCE].Value = 1; //turn both analog channels on if selecting a&b trig mode
        if (Title[TRACK2][SOURCE].Value == 0)
          Title[TRACK2][SOURCE].Value = 1;
      ...
      {
        //full updates
        if ((Current == T_BASE) && (_Det == MODE) && ((Key_Buffer == K_INDEX_DEC) || (Key_Buffer == K_INDEX_INC)))
          goto BypassExclusions;
        //note inclusions are 2 negates, eg: if current is not tbase, set bit to not update base
        //update base inclusions
        if (!((Current == T_BASE) && (_Det == BASE) && ((Key_Buffer == K_INDEX_DEC) || (Key_Buffer == K_INDEX_INC))))
          SelectiveUpdate |= 0x04;
        //update trigger inclusions
        if (!((((Current == TRIGG) && ((Title[TRIGG][SOURCE].Value < 4) || (_Det < 2))) || (Current < 4) || ((Current == T_VERNIE) && (Title[TRIGG][SOURCE].Value < 4))) && ((Key_Buffer == K_INDEX_DEC) || (Key_Buffer == K_INDEX_INC) || (Key_Buffer == K_INDEX_S))))
          SelectiveUpdate |= 0x10;
        //update range inclusions
        if (!((Current < 4) && (_Det < 3) && ((Key_Buffer == K_INDEX_DEC) || (Key_Buffer == K_INDEX_INC))))
          SelectiveUpdate |= 0x02;
      ...
      BypassExclusions:
        ...
        {
          if ((_Status == RUN) && ((((_Mode == NORH) || (_Mode == NORHLD)) && (__Get(FIFO_START) == 0)) || (_T_base < 8)))
          {                                   //trying to shift X position with device not triggered in run mode or at very slow speeds
            _X_posi.Value = GetXpos(XposRef); //align interpolated time bases with the others
            _State.Value = 1;                 //shift to hold mode so data is not transfered
            Process();                        //update screen so x pos can be changed
            _State.Value = 0;                 //return to run mode
      ...
      {
        if (!((_Kind == 8) && (StartUart)))
          SelectiveUpdate |= 0x08;
      }
      else if (_Det == 1)
        AlignTbaseSweep(2); //if changing frequencies, if in sweep/detector mode
    BypassUpdate:

      if ((Current == OUTPUT) && (_Det == 0) && ((Key_Buffer == K_INDEX_DEC) || (Key_Buffer == K_INDEX_INC)))
        Update_Output();
      ...
      if ((OSBufferLogic() != OSStatus))
      {
        if (OSBufferLogic())
        {
          if ((Current != FILE) && (_Curr[2].Value != BUF))
            InitiateOSBuffers();
          PrevKind = 0xFF;
        }
        Update_Base();
...
void EnableMeter(void)
{
  u8 i;
  if (UartLogic() == 0)
  { //enabling this causes T cursors to be moved to the left of meters when they are engaged
    if (Title[T_VERNIE][T1].Value > Title[T_VERNIE][T1].Limit - 95)
      Title[T_VERNIE][T1].Value = Title[T_VERNIE][T1].Limit - 95;
    if (Title[T_VERNIE][T2].Value > Title[T_VERNIE][T2].Limit - 95)
      Title[T_VERNIE][T2].Value = Title[T_VERNIE][T2].Limit - 95;
  }
...
void UpdateMeterPage(void)
{
  ...
  case 2:
    ...
    for (CounterUpdate = 1; CounterUpdate < 9; CounterUpdate++)
    {
      ...
      if (CounterUpdate < 6)
        Meter[CounterUpdate].Track = 2;
      else
        Meter[CounterUpdate].Track = 3;
    }
...
u8 ConfigFile(u8 service)
{
  ...
  { //save config
    ...
    if (FrameMode == 2)
    {
      ...
      SaveShortBuffXpos = ShortBuffXpos;
    ...
    Title[FILE][0].Value = LOAD; //preset file menu to load config files
    if (i == 0)
      ConfigFileNumber = Title[FILE][1].Value; //file number
    Title[FILE][2].Value = CFG;
  }
  else
  { //load config
...
void UpdateTriggerMem(void)
{
  ...
  for (j = 3; j > 0; j--)
    PreviousTrigSelect[j] = PreviousTrigSelect[(j - 1)];
  PreviousTrigSelect[0] = 0;
}

void SelUpdate(u8 detail)
{
  if ((Current < 2) && (_Det == detail))
    SelectiveUpdate |= (0x20 + (Current * 0x20));
}
...
void ChannelStatus(u8 service)
{
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    { //restore channel status
      ...
      ChStatus[i] = Title[i][SOURCE].Value; //save channel status
  }
}
...
{
  u8 j, k;
  if (PreviousTrigSelect[0] == i)
    PreviousTrigSelect[0] = 0; //clear to default (ch1) first entry if last ch memorized shut down
  for (j = 1; j < 4; j++)
  {
    if (PreviousTrigSelect[j] == i)
      for (k = j; k > 0; k--)
        PreviousTrigSelect[k] = PreviousTrigSelect[k - 1];
  }
}
...
void PWMscaleReset(u8 Dir)
{
  ...
  if ((Sweep != 1) && (PWAdjustMode == 1))
  {
    if ((_Frqn > 10) || ((_Frqn > 9) && (Dir == 1)))
    {
      ...
      { //preserve negative pulses (duty%>50)
        if (TIM4->CCR1 >= (D_Tab[_Frqn].ARR / 2) - 1)
          TIM4->CCR1 = (D_Tab[_Frqn].ARR / 2) - 1;
      }
      else
      { //positive pulses (duty%<50)
        Tmp = TIM4->ARR - TIM4->CCR1;
        Tmp = D_Tab[_Frqn].ARR - Tmp;
        if (Tmp < (D_Tab[_Frqn].ARR / 2))
          Tmp = (D_Tab[_Frqn].ARR) / 2;
        TIM4->CCR1 = Tmp;
      }
    }
    else
    {
      ...
      { //negative pulses
        if (Dir == 0)
        { //shifting freq range up
          Tmp = ((TIM4->CCR1 * (D_Tab[_Frqn - 1].PSC + 1)) / (D_Tab[_Frqn].PSC + 1));
          if (Tmp < (D_Tab[_Frqn].ARR / 2) - 1)
            TIM4->CCR1 = Tmp;
          else
            TIM4->CCR1 = (D_Tab[_Frqn].ARR / 2) - 1;
        }
        if (Dir == 1)
        { //shifting freq range down
          Tmp = ((TIM4->CCR1 * (D_Tab[_Frqn + 1].PSC + 1)) / (D_Tab[_Frqn].PSC + 1));
          if (Tmp > 0)
            TIM4->CCR1 = Tmp;
          else
      ...
      { //positive pulses
        if (Dir == 0)
        {
          ...
          if (Tmp < (D_Tab[_Frqn].ARR / 2))
            TIM4->CCR1 = (D_Tab[_Frqn].ARR - (Tmp + 1));
          else
            TIM4->CCR1 = (D_Tab[_Frqn].ARR / 2);
        }
        if (Dir == 1)
        {
...
void SetIRQ2Priority(void)
{
  ...
  if (Sweep == 1)
  { //SWEEP MODE
    if (((_Kind == 0) || (_Kind == 2)) && (SweepIndex > 7) && (_Frqn == 13))
    {
      pre = 0;
      sub = 1;
    } //20Khz sine, saw
...
void ProcessEditName(void)
{
  ...
  for (i = 0; i < 8; i++)
  {
    ...
    if (((LastAccessedConfig[i] > 64) && (LastAccessedConfig[i] < 91)) ||
        ((LastAccessedConfig[i] > 47) && (LastAccessedConfig[i] < 58)))
      EditChar[i][0] = LastAccessedConfig[i];
    else
      switch (LastAccessedConfig[i])
      {
      case 32:
      case 95:
      case 45:
      ...
      case 37:
        ...
        break;
      default:
        EditChar[i][0] = 63; //63(dot)= flag to use original char in LastAccessedConfig
      }
  }
}
...
void(UpdateFrameMode)(void)
{
  ...
  { //short buffer mode
    ...
    {
      if (OsBufferLogicFlag)
      { //note this is only active in non interpolated ranges, so KP is not a factor
        if (_X_posi.Value > 150)
          _X_posi.Value = 150; //limit xpos shift to left of screen in oversampling mode
        if (_T_base < 16)
          Tmp = (1208 - (_X_posi.Value * 8)) + (StartOffset * 8);
        else
          Tmp = (908 - (_X_posi.Value * 6)) + (StartOffset * 6);
  ...
  {    // shift trigger "splice" to left of screen
    ...
    if ((SubIndex == 0) || (_Mode != SCAN) || (_T_base > 0))
    {
      ...
      {
        if (XposFlag == 0)
        { //if flag set and in fast scan modes do nothing after going once in block and setting xpos to 150
          _X_posi.Value = 150;
          Update = 1;
          XposFlag = 1;
        }
      ...
      { //with flag set and all modes but scan with faster time bases
        if (_Mode == SCAN)
          _X_posi.Value = 0; //don't reset xpos if not in scan mode
        XposFlag = 0;
        Update = 1;
      }
      else if (_Mode == SCAN)
      { //if flag not set, after going into above and setting flag
        if (_X_posi.Value > 0)
        {                    //will drop in here as long as in scan mode with slower time base
